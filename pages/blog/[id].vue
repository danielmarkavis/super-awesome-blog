  ...
  <HomeLayout title="Home" description="Homepage">
    <template #content>
      <div class="container mx-auto py-5 md:max-w-lg">
        <div class="mb-5">
          <h1 class="text-2xl font-bold mb-4 dark:text-gray-300">
            {{ record?.title }}
          </h1>
        ...
        <h3
          class="text-xl font-bold dark:text-gray-400 mb-3 pt-3 border-t border-gray-600"
        >
          Comments
        </h3>
        <div v-for="(comment, index) in comments" :key="index" class="mb-3">
          <div class="flex">
            ...
            <div>
              <h4 class="text-lg font-bold dark:text-gray-300">
                {{ comment.name }}
              </h4>
              <p class="dark:text-gray-400">
                {{ comment.body }}
...
fetchArticle(id).then((json) => {
  record.value = Object.keys(json).length !== 0 ? json : null
})

const comments = ref(null)
fetchArticleComments(id).then((json) => {
